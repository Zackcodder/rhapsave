// ignore_for_file: constant_identifier_names, prefer_const_constructors

import 'package:flutter/material.dart';

const kBackground = Color(0xffF5F5F5);
const kNewHeaderColor = Color(0xff120A00);
const kGeneralbodytextColor = Color(0xff484848);
const kPorfileColor = Color(0xffF64F00);
const kCircleColor = Color(0xffFFFFFF);
const kverfiyColor = Color(0xffB2B3B3);
const kNavibuttonColor = Color(0xff828282);
const kBodyColor = Color(0xff484848);

const kNormaltextColor = Color(0xff26202C);
// const kSecndaryColor = Color(0xff52575C);

